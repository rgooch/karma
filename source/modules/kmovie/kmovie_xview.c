/*  kmovie_xview.c
    ...
    XView setup file for  kmovie  (X11 movie display tool for Karma).
...
#define DEFAULT_ANIMATE_INTERVAL 100
#define MIN_ANIMATE_INTERVAL 0
...
static void animate_button ();
static void start_animate_button (/* item, event */);
static void stop_animate_button (/* item, event */);
static void animate_slider_proc ();
static Notify_value animate_timer_proc ();
static void load_cmap (/* multi_desc */);
static void prev_frame_button (/* item, event */);
static void next_frame_button (/* item, event */);
...
/*  Animate control panel info  */
static Frame animate_frame;
static int animate_interval = DEFAULT_ANIMATE_INTERVAL;
flag animate_on = FALSE;
static Panel_item framegrab_item;
static unsigned int frame_number = 0;

flag shimmer_on = FALSE;
...
char *argv[];
{
    ...
    static char function_name[] = "kmovie_xview";
...
*/
    ...
    (void) xv_create (panel, PANEL_BUTTON,
		      ...
		      PANEL_NOTIFY_PROC, animate_button,
		      NULL);
    (void) xv_create (panel, PANEL_BUTTON,
    ...
    cmd_frame = (Frame) xv_create (base_frame, FRAME_CMD,
				   FRAME_LABEL, "kmovie File Selector",
				   NULL);
    ...
    track_frame = (Frame) xv_create (base_frame, FRAME_CMD,
				     FRAME_LABEL, "kmovie Track Window",
				     FRAME_SHOW_RESIZE_CORNER, TRUE,
    ...
    /*  Create the Animate control frame  */
    animate_frame = (Frame) xv_create (base_frame, FRAME_CMD,
					  FRAME_LABEL,
					  "kmovie Animate Control Window",
					  NULL);
    /*  Command frames have panels already created by default: get it  */
    panel = (Panel) xv_get (animate_frame, FRAME_CMD_PANEL);
    /*  Create "Start Movie" and "Stop Movie" buttons  */
    (void) xv_create (panel, PANEL_BUTTON,
		      PANEL_LABEL_STRING, "Start Movie",
		      PANEL_NOTIFY_PROC, start_animate_button,
		      NULL);
    (void) xv_create (panel, PANEL_BUTTON,
		      PANEL_LABEL_STRING, "Stop Movie",
		      PANEL_NOTIFY_PROC, stop_animate_button,
		      NULL);
    (void) xv_create (panel, PANEL_BUTTON,
		      PANEL_LABEL_STRING, "Previous Frame",
		      PANEL_NOTIFY_PROC, prev_frame_button,
		      NULL);
    (void) xv_create (panel, PANEL_BUTTON,
		      PANEL_LABEL_STRING, "Next Frame",
		      PANEL_NOTIFY_PROC, next_frame_button,
		      NULL);
    /*  Create blink interval slider on panel  */
    (void) xv_create (panel, PANEL_SLIDER,
		      PANEL_LABEL_STRING, "Interval Between Blinks (ms)",
		      PANEL_VALUE, DEFAULT_ANIMATE_INTERVAL,
		      PANEL_MIN_VALUE, MIN_ANIMATE_INTERVAL,
		      PANEL_MAX_VALUE, MAX_ANIMATE_INTERVAL,
		      PANEL_NOTIFY_PROC, animate_slider_proc,
		      PANEL_NOTIFY_LEVEL, PANEL_ALL,
		      PANEL_NEXT_ROW, -1,
		      NULL);
    framegrab_item = (Panel_item) xv_create (panel, PANEL_TEXT,
					     PANEL_LABEL_STRING,
					     "FrameGrab command:",
					     PANEL_LAYOUT,
					     PANEL_HORIZONTAL,
					     NULL);
    /*  Size panel only to the required size (vertical size only)  */
    window_fit_height (panel);
    window_fit_height (animate_frame);
...
Event *event;
{
    ...
    image_auto_x = TRUE;
    image_auto_y = TRUE;
    if (image_auto_v) (void) fprintf (stderr, "AUTO_V SCALING!!!\n\n");
    viewimg_control_autoscaling (image_worldcanvas,
				 image_auto_x, image_auto_y, image_auto_v,
				 TRUE, TRUE, maintain_aspect_ratio);
    /*  Redraw the canvas  */
...
Xv_xrectlist *rects;
{
	...
	plotted_scale.z_min = 0.0;
	plotted_scale.z_max = 255.0;
...
void kmovie_init (display)
/*  This routine will initialise the kmovie module.
...
char *arrayfile;
{
    ...
    {
	if ( ( cube = iarray_get_from_multi_array (multi_desc, "Movie", 3,
						   (char **) NULL, NULL) )
	    == NULL )
	{
	    (void) fprintf (stderr,
			    "Error getting Intelligent Array: Movie\n");
	    ds_dealloc_multi (multi_desc);
	    return (FALSE);
	}
	...
	if ( ( cube = iarray_get_from_multi_array (multi_desc, NULL, 3,
	    ...
	    == NULL )
	{
	    (void) fprintf (stderr, "Error getting Intelligent Array\n");
	    ds_dealloc_multi (multi_desc);
	    return (FALSE);
	}
    }
    cube_desc = multi_desc;
    /*  Deallocate old ViewableImage objects  */
    for (count = 0; count < num_frames; ++count)
    {
	viewimg_destroy (frames[count]);
    ...
    {
	/*  Have colourmap  */
	plotted_scale.z_min = 0;
	plotted_scale.z_max = kcmap_get_pixels (image_cmap,
						(unsigned long **) NULL) - 1;
    }
    else
    {
	if (iarray_min_max (cube, CONV1_REAL,
			    &(plotted_scale.z_min), &(plotted_scale.z_max))
	    != TRUE)
	{
	    (void) fprintf (stderr, "Error computing min-max\n");
	}
    }
    if (canvas_resize (image_worldcanvas, &plotted_scale, FALSE) != TRUE)
    {
...
/*  Animate functions follow  */

static void animate_button (item, event)
/*  This routine will register that the auto-blink button was pressed.
    It will show the auto-blink control panel.
    The routine returns nothing.
*/
Panel_item item;
Event *event;
{
    ...
    /*  Show the auto-blink control panel frame and push in the pushpin  */
    xv_set (animate_frame,
	    XV_SHOW, TRUE,
	    FRAME_CMD_PUSHPIN_IN, TRUE, NULL);
}   /*  End Function animate_button  */

static void start_animate_button (item, event)
/*  This routine will begin the movie.
    The routine returns nothing.
*/
Panel_item item;
Event *event;
{
    ...
    extern flag animate_on;
    extern flag shimmer_on;
    extern int animate_interval;

    if (shimmer_on)
    {
	show_error_message ("Cannot AutoBlink while ColourmapShimmering");
	return;
    }
    /*  Set the frame busy attribute  */
/*  Removed because this prevents processing of X events (like clicking on the
    "Quit"  button.
    xv_set (base_frame, FRAME_BUSY, TRUE, NULL);
*/
    if (animate_interval < 1)
    {
    ...
    {
	timer.it_value.tv_usec = (animate_interval % 1000) * 1000;
	timer.it_value.tv_sec = animate_interval / 1000;
    }
    timer.it_interval.tv_usec = 0;
    timer.it_interval.tv_sec = 0;
    notify_set_itimer_func (ANIMATE_TIMER_CLIENT, animate_timer_proc,
			    ITIMER_REAL, &timer, (struct itimerval *) NULL);
    animate_on = TRUE;
}   /*  End Function start_animate_button  */

static void stop_animate_button (item, event)
/*  This routine will stop the movie.
    The routine returns nothing.
*/
Panel_item item;
Event *event;
{
    ...
    /*  Unset the frame busy attribute  */
    xv_set (base_frame, FRAME_BUSY, FALSE, NULL);
    notify_set_itimer_func (ANIMATE_TIMER_CLIENT, NOTIFY_FUNC_NULL,
			    ITIMER_REAL,
			    (struct itimerval *) NULL,
			    (struct itimerval *) NULL);
    animate_on = FALSE;
}   /*  End Function stop_animate_button  */

static void animate_slider_proc (item, value, event)
/*  This routine will register a change in the auto-blink interval slider
    value.
    The routine returns nothing.
*/
Panel_item item;
...
Event *event;
{
    ...
    animate_interval = value;
}   /*  End Function animate_slider_proc  */

static Notify_value animate_timer_proc (client, which)
/*  This routine is called by the Notifier whenever the auto-blink timer times
    out.
    The routine returns nothing.
*/
Notify_client client;
int which;
{
	...
	start_animate_button ( (Panel_item) NULL, (Event *) NULL );
	return (NOTIFY_DONE);
    }
    if (++frame_number >= num_frames) frame_number = 0;
    (void) viewimg_make_active (frames[frame_number]);
/*
    load_cmap (cube_desc);
*/
    if ( ( command = (char *) xv_get (framegrab_item, PANEL_VALUE) ) != NULL )
    {
	(void) system (command);
    }
    /*  Start the timer again  */
    start_animate_button ( (Panel_item) NULL, (Event *) NULL );
    return (NOTIFY_DONE);
}   /*  End Function animate_timer_proc  */

static void load_cmap (multi_desc)
/*  This routine will attempt to load the colourmap.
...
multi_array *multi_desc;
{
    ...
    unsigned int dummy;
    extern Kcolourmap image_cmap;
    static char function_name[] = "load_cmap";
					  ...
					  (char **) NULL, &cmap_index) )
	{
	  case IDENT_NOT_FOUND:
	    (void) fprintf (stderr, "Colourmap not found\n");
	    break;
...
}   /*  End Function load_cmap  */

static void prev_frame_button (item, event)
/*  This routine will show the previous frame.
    The routine returns nothing.
*/
Panel_item item;
Event *event;
{
    ...
    return;
}   /*  End Function prev_frame_button  */

static void next_frame_button (item, event)
/*  This routine will show the next frame.
    The routine returns nothing.
*/
Panel_item item;
Event *event;
{
    ...
    return;
}   /*  End Function next_frame_button  */
