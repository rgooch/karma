/*  karma2miriad.c
    ...
    Source file for  karma2miriad  (Karma to Miriad Image conversion module).
...
/*  This Karma module will read in a Karma data file and convert the
    n-dimensional array to a Miriad Image file.
...
STATIC_FUNCTION (flag karma2miriad, (char *command, FILE *fp) );
...
int main (int argc, char **argv)
{
    ...
    module_run (argc, argv, "karma2miriad", VERSION, karma2miriad, -1, -1,
		FALSE);
    return (RV_OK);
}   /*  End Function main   */

static flag karma2miriad (p, fp)
char *p;
FILE *fp;
{
    ...
    if ( !foreign_miriad_write (out_filename, multi_desc,
				FA_MIRIAD_WRITE_END) )
    {
	(void) fprintf (stderr, "Error writing Miriad Image file\n");
