...
static XtResource TwodposResources[] = 
{
  ...
  {XkwNminimum_x, XkwCMinimum_x, XtRFloat, sizeof(double),
    TheOffset(minimum_x), XtRFloat, (XtPointer)&zero},
  {XkwNmaximum_x, XkwCMaximum_x, XtRFloat, sizeof(double),
    TheOffset(maximum_x), XtRFloat, (XtPointer)&one},
  {XkwNvalue_x, XkwCValue_x, XtRPointer, sizeof(double),
    TheOffset(value_x), XtRPointer, (XtPointer)&half},
  {XkwNminimum_y, XkwCMinimum_y, XtRFloat, sizeof(double),
    TheOffset(minimum_y), XtRFloat, (XtPointer)&zero},
  {XkwNmaximum_y, XkwCMaximum_y, XtRFloat, sizeof(double),
    TheOffset(maximum_y), XtRFloat, (XtPointer)&one},
  {XkwNvalue_y, XkwCValue_y, XtRPointer, sizeof(double),
    TheOffset(value_y), XtRPointer, (XtPointer)&half},
...
static void check_values(double *min,double *max,double *value)
{
...
static void Resize(Widget W)
{
  ...
  w->twodpos.scale_x=
    (w->twodpos.maximum_x-w->twodpos.minimum_x)/(double)w->core.width;
  w->twodpos.scale_y=
    (w->twodpos.maximum_y-w->twodpos.minimum_y)/(double)w->core.height;
}
...
static void Set(Widget W,XEvent *event,String *argv,int *argc)
{
  ...
  switch (event->type)
  {
    case ButtonPress:
    case ButtonRelease:
      x = (double)((XButtonEvent *)event)->x;
      y = (double)((XButtonEvent *)event)->y;
      break;
    case KeyPress:
    case KeyRelease:
  ...
  w->twodpos.value_x=(double)x*w->twodpos.scale_x+w->twodpos.minimum_x;
  w->twodpos.value_y=(double)y*w->twodpos.scale_y+w->twodpos.minimum_y;
