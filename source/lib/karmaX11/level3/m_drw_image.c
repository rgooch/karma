...
/*
  ...
  structure. This code is only compiled on platforms where misaligned data
  accesses require a special compile-time switch.


    Written by      Richard Gooch   3-OCT-1992

    Updated by      Richard Gooch   2-MAR-1993

    Last updated by Richard Gooch   13-APR-1993: Used change to  win_scale
  structure.
...
#if defined(NEEDS_MISALIGN_COMPILE)
...
flag misalign__drw_single_plane (ximage, num_pixels, pixel_values,
...
struct win_scale_type *win_scale;
{
    ...
    char *image_ptr;
    static char function_name[] = "misalign__drw_single_plane";

    (void) fprintf (stderr, "\n%s started", function_name);
    if (num_pixels < 1)
    {
	return (FALSE);
    }
    ...
    {
	(void) fprintf (stderr, "\nError getting resource usage\t%s%c",
	...
	( (*win_scale).x_log == FALSE ) && ( (*win_scale).y_log == FALSE ) )
    {
    ...
    (void) fprintf (stderr, "\n%s ended\tcpu time: %lu msecs",
		    function_name, cpu_sec * 1000 + cpu_usec / 1000);
    (void) fprintf (stderr, "\nNot finished slow draw yet%c", BEL);
    return (FALSE);
}   /*  End Function misalign__drw_single_plane  */
...
unsigned int z_scale;
{
	...
	break;
      default:
	(void) fprintf (stderr, "\nIllegal value of intensity scale: %u",
			z_scale);
	a_prog_bug (function_name);
	break;
    }
...
unsigned int z_scale;
{
	...
	break;
      default:
	(void) fprintf (stderr, "\nNot finished various scale types");
	return (FALSE);
	break;
    }
...
unsigned long max_sat_pixel;
{
	...
	break;
      default:
	(void) fprintf (stderr, "\nIllegal intensity scale value: %d", scale);
	a_prog_bug (function_name);
	break;
    }
