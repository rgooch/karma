...
/*
  ...
  with network data format.
    ...
    Last updated by Richard Gooch   14-SEP-1993: Fixed bug in
...
Channel channel;
{
	...
	== NULL )
    {
	(void) fprintf (stderr, "Error reading history\n");
	ds_dealloc_multi (multi_desc);
	return (NULL);
    }
    while (length > 0)
...
flag pad;
{
	...
	if (block_transfer)
	{
	    ...
	    *( (unsigned int *) ( element + sizeof (int) ) ) = K_ARRAY_MMAP;
	...
	{
	    if (ds_alloc_array (descriptor, element, FALSE, FALSE) != TRUE)
	    {
