...
/*  This routine will search a Karma general data structure for an instance of
    ...
    If the data is not re-ordered, or if this is NULL, then nothing is written
    here.
    ...
    which each contain first a red intensity, then a blue and then a green.
*/
packet_desc *top_pack_desc;
char *top_packet;
unsigned int *size;
...
*/
    ...
    if ( (reordering_done != NULL) && (reorder_needed == TRUE) )
    {
	*reordering_done = TRUE;
    }
    while (reorder_needed == TRUE)
    {
	/*  Need to re-order the data  */
	reorder_needed = FALSE;
	for (elem_count = 0;
	     (elem_count < NUM_ELEMENTS - 1) && (reorder_needed != TRUE);
	     ++elem_count)
	{
	    if (name_indices[elem_count] != elem_count)
	    {
		/*  Out of position  */
		reorder_needed = TRUE;
	    }
