...
/*
    ...
    Updated by      Richard Gooch   7-APR-1993

    Updated by      Richard Gooch   26-NOV-1994: Moved to  packages/ds/list.c
...
/*PUBLIC_FUNCTION*/
void ds_list_insert (list_header *list_head, list_entry *new_entry,
		     list_entry *entry)
/*  [SUMMARY] Insert an entry into the fragmented section of a linked list.
    ...
    <entry> The list entry which is to be moved beyond the new entry. If this
...
/*PUBLIC_FUNCTION*/
void ds_list_delete (packet_desc *list_desc, list_header *list_head,
		     list_entry *entry)
/*  [SUMMARY] Delete an entry from the fragmented section of a linked list.
    <list_desc> The packet descriptor for the list entry data. If this is not
    NULL, the routine will recursively deallocate the packet data.
    <list_head> The list header.
    <entry> The entry to be deleted. The entry and it's data packet are
    deallocated.
    [RETURNS] Nothing.
*/
{
...
/*  [SUMMARY] Unfragment a linked list.
    [PURPOSE] This routine will unfragment a linked list (i.e. all the entries
...
/*  [SUMMARY] Fragment a linked list.
    [PURPOSE] This routine will fragment a linked list (i.e. all the contiguous
...
flag increasing;
{
    ...
    FLAG_VERIFY (increasing);
    elem_type = list_desc->element_types[elem_num];
    if ( !ds_element_is_atomic (elem_type) )
    {
	return;
    }
    if ( ( ( (list_head->sort_type == SORT_INCREASING) && increasing ) ||
	   ( (list_head->sort_type == SORT_DECREASING) && !increasing ) ) &&
    ...
    /*  Yeah, yeah, yeah. Bubble sort, I know. It's not compiled anyway.  */
	...
	/*  List needs to be passed over again  */
	    ...
	    entry2 = entry1->next;
            ...
            if ( ( increasing && (value1 > value2) ) ||
		 ( !increasing && (value2 > value1) ) )
            {
		/*  Must swap  */
