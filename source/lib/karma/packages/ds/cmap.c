...
/*PUBLIC_FUNCTION*/
unsigned short *ds_cmap_alloc_colourmap (unsigned int size,
					 multi_array **multi_desc,
					 packet_desc **pack_desc,char **packet)
/*  [SUMMARY] Allocate a Karma data structure to hold a colourmap.
    [PURPOSE] This routine will allocate a colourmap which comforms to the
    ...
    <multi_desc> The pointer to the multi array header which is allocated will
...
*/
{
    ...
    if (pack_desc != NULL) *pack_desc = top_pack_desc;
    if (packet != NULL) *packet = top_packet;
    return (*(unsigned short **) top_packet);
}   /*  End Function ds_cmap_alloc_colourmap  */

/*PUBLIC_FUNCTION*/
unsigned short *ds_cmap_find_colourmap (packet_desc *top_pack_desc,
...
/*  [SUMMARY] Search a Karma data structure for an instance of a colourmap.
    ...
    <size> The size of the colourmap (number of colours) in the colourmap will
    be here.
    <reordering_done> The routine is tolerant of incorrect ordering of the
...
/*PUBLIC_FUNCTION*/
unsigned int *ds_cmap_get_all_colourmaps (multi_array *multi_desc,
...
/*  [SUMMARY] Search an array of Karma data structures for colourmaps.
    [PURPOSE] This routine will search an array of Karma general data
    structures for instances of a colourmap.
    <multi_desc> The multi_array descriptor.
    <num_found> The number of general data structures which contain a
...
*/
{
    ...
    if (reordering_done != NULL) *reordering_done = FALSE;
    *num_found = 0;
