...
/*  Private functions  */
static XtInputCallbackProc xt_input_handler (/* client_data, fd, id */);
...
flag (*exception_func) ();
{
    ...
    if ( (output_func != NULL) || (exception_func != NULL) )
    {
	(void) fprintf (stderr, "Output and exception events not finished\n");
	a_prog_bug (function_name);
    }
    ...
    {
	id = XtAppAddInput (app_context, fd, XtInputReadMask, xt_input_handler,
			    (XtPointer) new_entry);
	(*new_entry).id = id;
    }
#ifdef dummy
...
Channel channel;
{
	...
	if (channel == (*entry).channel)
	{
	    ...
	    {
		(void) fprintf (stderr,
				"%s: WARNING: no read funcs registered\n",
				function_name);
	    }
	    if ( (*entry).prev == NULL )
	    {
